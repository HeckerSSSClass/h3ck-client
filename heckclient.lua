...
Converted["_YARHM"].Name = "Heck Client"
Converted["_YARHM"].Parent = game:GetService("CoreGui")

Converted["_Open"].Font = Enum.Font.Gotham
Converted["_Open"].Text = "Triple-click this region to open Heck Client."
...
Converted["_HubCredits"].Text = "Beta version"
...
Converted["_HubDesc"].Text = "Heck Client - Ez Win!"
...
Converted["_HubName"].Text = "Heck Client"
...
Converted["_TextLabel5"].Text = "Heck Client"
...
Converted["_TextLabel7"].Text = "Heck Client"
    ...
    local function module_script()
		...
		function FUNCTIONSmodule.refreshlist()
			...
			for modulei, module in ipairs(_G.Modules) do
				...
				if not success then
					warn("[Heck Client] Error while requiring " .. module.Name .. "!")
					warn(err)
				end
			end
		end
...
local function VXLACE_fake_script() -- Fake Script: StarterGui.YARHM.Init
	...
	local s, e = pcall(function()
		if get_hidden_gui or gethui then
			...
			print("[Heck Client] - Using get_hidden_gui for anti-detection.")
		elseif (not is_sirhurt_closure) and (syn and syn.protect_gui) then
			script.Parent.Name = randomString()
			syn.protect_gui(script.Parent)
			script.Parent.Parent = COREGUI
			print("[HeckClient] - Using syn.protect_gui for anti-detection.")
		elseif COREGUI:FindFirstChild('RobloxGui') then
			script.Parent.Parent = COREGUI.RobloxGui
			print("[Heck Client] - Using RobloxGui for anti-detection.")
		else
			warn("[Heck Client] - Using CoreGui as anti-detection. This is the most basic coverage and can still be detected.")
		end
	end)
	
	print("[YARHM] - YARHM is now in " .. tostring(script.Parent:GetFullName()))
	if not s then
		warn("[Heck Client - Attempts at anti-detection failed. Using CoreGui as anti-detection.")
		warn(e)	
	end
	
	printidentity("[Heck Client] - Your executor level (identity) is")
	
	local getExeName = identifyexecutor or getexecutorname or function() return "Yet Another Roblox Executor v1.2" end
	print("[Heck Client] - Your executor is " .. getExeName())
			...
			Auto_Shoot_murderer = function()
				...
				if findSheriff() == localplayer and autoShooting then
					...
					repeat
						task.wait(0.1)
						local murderer = findMurderer() or findSheriffThatsNotMe()
						if not murderer then warn("[Heck Client] > MM2 Autoshoot - No murderer.") continue end
						local murdererPosition = murderer.Character.HumanoidRootPart.Position
						local characterRootPart = localplayer.Character.HumanoidRootPart
